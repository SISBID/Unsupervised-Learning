...
Dmat = dist(t(X)) #Euclidean distance matrix
mdsres = cmdscale(Dmat,k=2)

par(mfrow=c(1,2))
plot(mdsres[,1],mdsres[,2],type="n")
text(mdsres[,1],mdsres[,2],colnames(X),col=cols)

plot(U[,1],U[,2],type="n",xlab="PC1",ylab="PC2")
text(U[,1],U[,2],colnames(X),col=cols) #same thing!


######
Dmat = dist(t(X),method="manhattan") #Manhattan (L1) Distance
mdsres = cmdscale(Dmat,k=2)
